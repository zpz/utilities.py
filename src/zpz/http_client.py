...
async def _a_request(func, url, **kwargs):
    ...
    try:
        ...
        timeout_duration = time1 - time0
        logger.error(
            "HTTP request failed after %d seconds with %s: %s",
            timeout_duration,
...
class HTTPStatusError(Exception):
    def __init__(self, e: _HTTPStatusError):
        super().__init__(*e.args)
        self.request = e.request
        self.response = e.response
        self.status_code = e.response.status_code
        self.is_client_error = httpx.codes.is_client_error(
            e.response.status_code)
        self.is_server_error = httpx.codes.is_server_error(
            e.response.status_code)
    ...
    if method == 'get':
        ...
        if payload:
            if isinstance(payload, bytes):
                ...
                args = {'data': io.BytesIO(payload.encode())}
                payload_type = 'text/plain'
            else:
                if payload_type == 'json':
                    args = {'json': payload}
